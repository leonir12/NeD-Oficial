...
class CadastrosController {
    static async pegaTodosOsCadastros(req,res) {
     ...
     static async pegaUmCadastro(req, res) {
        const { id } = req.params;
        try{
            const umCadastro = await database.Cadastro.findOne( { 
                where: { 
                    id: Number(id) 
                } 
            });
    ...
    static async criaCadastro(req, res) {
        ...
        try{
            const novoCadastroCriado = await database.Cadastro.create(novoCadastro);
    ...
    static async atualizaCadastro(req, res) {
        ...
        const { id } = req.params;
        try{
            await database.Cadastro.update(novasInfosCadastro, { where: { id: Number(id) } });
            const cadastroAtualizado = await database.Cadastro.findOne( { where: { id: Number(id) } });
